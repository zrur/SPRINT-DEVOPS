...
v_missing INT;
  ...
  PROCEDURE add_col(p_sql VARCHAR) IS
BEGIN
...
WHERE table_name = 'TB_CLIENTE'
  AND column_name = 'CPF';
IF v_missing = 0 THEN
    add_col('ALTER TABLE TB_CLIENTE ADD (CPF VARCHAR(14))');
END IF;
...
IF v_missing = 0 THEN
    add_col('ALTER TABLE TB_CLIENTE ADD (DATA_CADASTRO DATETIME)');
END IF;
...
IF v_missing = 0 THEN
    add_col('ALTER TABLE TB_CLIENTE ADD (EMAIL VARCHAR(120))');
END IF;
